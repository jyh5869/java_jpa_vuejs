...
model.add(LSTM(64))
model.add(Dense(len(addresses_processed), activation='softmax'))  # 도로명 개수에 맞는 Dense layer로 수정
...
try:
    num_epochs = 2
    model.fit(padded_sequences, np.array(range(len(sequences))), epochs=num_epochs, verbose=1)  # 출력값을 도로명 인덱스와 일치하도록 수정
...
loss, accuracy = model.evaluate(padded_sequences, np.array(range(len(sequences))), verbose=1)
...
def predict_top_similar_addresses(input_address, top_n=10):
    ...
    top_predicted_addresses = [addresses[idx] for idx in top_predicted_indices]
...
top_similar_addresses = predict_top_similar_addresses(test_address)
