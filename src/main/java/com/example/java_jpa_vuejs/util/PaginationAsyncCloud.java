...
*/
		...
		for(int i = intPageGroupStart; i < intPageGroupEnd; i++){
			...
			System.out.println("＠ @ @ @ @ @  @ @ @ =  arr[pageNum] : " + arr[i] + "  //  strCurrensPage : " + strTargetDoc );
			
			//if(arr[pageNum].equals(strTargetDoc) || i == intCurrentPage){//선택된 페이지 일경우
			if(i == intCurrentPage){//선택된 페이지 일경우
				...
				strPagingBuf.append(makeLinkByParams(String.valueOf(arr[i+1])+"|"+strPageNum+"|EACH", "" + String.valueOf((pageLinkNm+1)) + "",  strParams, strActionUrl, intCountPerPage, docIdArr));
			}
		}
		...
		if((intPageGroupEnd * intCountPerPage) < intTotalCount){//호출 페이지가 마지막 페이지를 넘어가지 않을때(일반적인 다음페이지 호출)
			...
			strPagingBuf.append(makeButtonLinkByParams("1|"+String.valueOf(intPageGroupEnd)+"|NEXT"  , "NEXT_TAG", strParams, strActionUrl, intCountPerPage, docIdArr));
		}
			...
			strPagingBuf.append(makeButtonLinkByParams("1|"+String.valueOf(intPageGroupEnd-1)+"|NEXT" , "NEXT_TAG", strParams, strActionUrl, intCountPerPage, docIdArr));
		}
    ...
    private static String makeButtonLinkByParams(final String strDocId, final String strLinkImg, final String strParams, final String actionUrl, final int countPerPage, final String docIdArr){
		 ...
		 */
		strLinkBuf.append("('").append(strDocId).append("',").append(countPerPage).append(",'").append(strParams).append("','")
